...
export const useGetButtonMatrix = (lastButtonRef: React.RefObject<HTMLButtonElement>, adjustWidth: (increaseWidthBy: number) => void): IButtonData[][] => {
        ...
        if (operationsLoaded) {
            ...
            const generateSpecialOperators = () => {
                ...
                for (let i = 0; i < filteredSpecialOperators.length; i++) {
                    ...
                    currentRowContainer.push({ content: filteredSpecialOperators[i], type: ButtonType.SPECIAL_OPERATOR })
                }
            ...
            const generateNumericButtons = () => {
                ...
                for (let i = calculatorViewConstants.MIN_BUTTON_VALUE; i <= calculatorViewConstants.MAX_BUTTON_VALUE; i++) {
                    currentRowContainer.push({ content: `${i}`, type: ButtonType.NUMERIC })

                    if (i % calculatorViewConstants.NUMBERS_COLUMNS_AMOUNT === 0 && operatorIndex < basicOperators.length) {
                        currentRowContainer.push({ content: `${basicOperators[operatorIndex]}`, type: ButtonType.OPERATOR })
                        operatorIndex++
                        isRowCompleted = true;
                    }
            ...
            const generateZeroAndOperators = () => {
                currentRowContainer.push({ content: `${SpecialOperators.DOT}`, type: ButtonType.NUMERIC })
                buttonCounter = 1;

                currentRowContainer.push({ content: `${0}`, type: ButtonType.NUMERIC })
                currentRowContainer.push({ content: `${EVALUATE}`, type: ButtonType.EVALUATE })

                currentRowContainer.push({ content: `${operators[operatorIndex]}`, type: ButtonType.OPERATOR })
                operatorIndex++;
            }
            ...
            const generateRemainingOperators = () => {
                ...
                remainingOperators.forEach((operator, idx) => {
                    rowArrayContainer[index].unshift({ content: `${operator}`, type: ButtonType.OPERATOR, isLastButton: idx === remainingOperators.length - 1 })
                    index++;
                    ...
                    if (index >= rowArrayContainer.length) {
                        ...
                        if (isLastButton && isRowIncomplete && lastButtonRef.current) {
                            ...
                            const height = amountOfRowsLeft * lastButtonCoords.height + calculatorViewConstants.GAP + calculatorViewConstants.GAP / 2
