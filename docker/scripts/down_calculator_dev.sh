...
if [ -f "$ENV_FILE" ]; then
    ...
    if [ "$DATABASE" == "mongoDB" ]; then
        echo "Running docker-compose-dev for MongoDB"
        docker-compose -f ../docker-compose-dev.yml down client server mongo
    elif [ "$DATABASE" == "postgreSQL" ]; then
        echo "Running docker-compose-dev for PostgreSQL"
