...
export class PostgresCalculationModel extends BaseKnexModel implements ICalculationModel {
    constructor() {
    ...
    public async findOne(query: Partial<IExpression>): Promise<IExpression | null> {
        const result = await this.findBy<Partial<IExpression>, IExpression>(query);
        return result
    }
